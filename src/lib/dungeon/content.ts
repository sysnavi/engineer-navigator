...
export type Monster = {
  ...
  weight: number; // 抽選の重み
  boss?: boolean;
  /** 得意領域。戦闘で出す問題のお題（QuizQuestion.topic / riddles.topics）をこれで絞る。
   *  語彙は良問バンクに実在する topic に揃える（ゆるい包含で照合するので "AWS" は "AWS IAM" にも当たる） */
  topics: string[];
...
export const MONSTERS: Monster[] = [
  {
    ...
    weight: 10,
    topics: ["テスト", "Git"],
    encounter: "小さなバグが飛び出してきた！ 「これ、どこが変？」",
    win: "ワンライナーで退治した。+1階",
    lose: "再現手順が分からず見失った…",
  },
  {
    ...
    weight: 10,
    topics: ["Git", "Linux"],
    encounter: "タイポの小人が足元にセミコロンを撒いた！ 「1文字ちがいを見抜けるか？」",
  ...
  },
  {
    ...
    weight: 8,
    topics: ["SQL", "テスト"],
    encounter: "オフバイワン鳥が1歩ずれて飛んでいる！ 「境界はどっち？」",
    win: "境界値を見切って捕まえた。+1階",
    lose: "最後の1羽が数えられなかった…",
  },
  {
    ...
    weight: 8,
    topics: ["HTTP", "Web"],
    encounter: "「縺ゅ→縺ｧ」と鳴く影が現れた！ 問いだけは読める。",
    win: "UTF-8の御札で祓った。+1階",
    lose: "何を言っているのか分からない…",
  },
  {
    ...
    weight: 8,
    topics: ["設計", "テスト"],
    encounter: "自分の尻尾を追うヘビが道を塞いでいる！ 「抜け道を答えろ」",
  ...
  },
  {
    ...
    weight: 8,
    topics: ["設計", "テスト"],
    encounter: "実体のない何かがそこに「無い」！ 問いだけが浮かんでいる。",
  ...
  },
  {
    ...
    weight: 8,
    topics: ["Linux", "Docker"],
    encounter: "スライムがじわじわ膨らみ続けている！ 「解放の呪文は？」",
    win: "解放の呪文でしぼませた。+1階",
  ...
  },
  {
    ...
    weight: 7,
    topics: ["SQL", "設計"],
    encounter: "2匹のカニが互いを挟んで動けなくなっている！ 「どちらが先か、答えろ」",
  ...
  },
  {
    ...
    weight: 7,
    topics: ["HTTP", "Web"],
    encounter: "倒したはずの敵の残像が立ちはだかる！ 「本物はどれだ？」",
  ...
  },
  {
    ...
    weight: 7,
    topics: ["テスト", "プロジェクト"],
    encounter: "たまにしか当たらない攻撃をするコウモリだ！ 「再現できるか？」",
    win: "3回リトライして見事命中。+1階",
  ...
  },
  {
    ...
    weight: 6,
    topics: ["プロジェクト", "設計"],
    encounter: "戦っている最中に姿が変わっていく！ 「いま正しいのは？」",
    win: "要件を書面で固定して撃破。+1階",
  ...
  },
  {
    ...
    weight: 6,
    topics: ["設計", "Git"],
    encounter: "放置された年月のぶんだけ硬いゴーレムが現れた！ 「基本から問おう」",
  ...
  // --- ボス（深層のみ・突破で+2階&SSR確定） ---
  {
    ...
    boss: true,
    topics: ["設計", "テスト", "Git"],
    encounter: "誰も全容を知らない巨竜が目を覚ました！ 「我を知る者だけが通れる」",
  ...
  },
  {
    ...
    weight: 8,
    boss: true,
    topics: ["セキュリティ", "Linux", "Docker", "AWS"],
    encounter: "首を1本直すと2本生える怪物が咆哮した！ 「根本原因を答えよ」",
    win: "根本原因の心臓を貫いた！+2階",
    lose: "対症療法では首は減らなかった…",
  },
];
