...
画面は5つ: `/`(ホーム) / `/report`(週報・自動保存つき) / `/skills`(スキルマップ＋レーダー＋成長ログ＋AI提案承認) / `/resume`(経歴書・印刷=PDF) / `/mypage`(きせかえ)。
**Phase 1 は完了**（2026-07-14）。全画面 8bit/Y2K デザイン + カラーパレットきせかえ（User.palette、5種）も実装・E2E検証済み。
...
1. Phase 2: コンディションダッシュボード（トレンド検知・営業/管理者向け）
2. 経歴書エクスポートの拡張（現状はブラウザ印刷=PDF。営業側フォーマットが決まれば合わせる）
3. 8bit演出の磨き込み（LEVEL UP時のアニメーション等・ユーザーFB待ち）
