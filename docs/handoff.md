...
- **おさんぽ（2026-07-25, Issue #26）**: うちの子と歩くのを眺めるだけの見る専画面 `/walk`。操作不要・低負荷。
  ...
  - **天気**: Open-Meteoへ**ブラウザから直fetch**（`src/lib/walk/weather.ts`）。**座標は当サービスのサーバーに送らない**。許可なし/失敗時は季節＋時刻の擬似天気に自動フォールバック（ボタンから押したときは理由を一言で表示）。アプリ殻では `NSLocationWhenInUseUsageDescription`（iOS）と `ACCESS_*_LOCATION`（Android）の宣言が必須。雨・雪・霧はcanvas内のパーティクル（`drawWeather`）。
...
- **ダンジョンBGM（`src/app/dungeon/bgm.tsx`・2026-09-11）**: 開発者本人の1曲 `public/bgm/dungeon-01.mp3` を潜行中ループ再生。おさんぽと違い**既定ON・端末に記憶**（潜るたびに押させると存在に気づかれない）。「▶ 潜る」のクリック内で play() を呼び始めるので自動再生制限に掛からない。潜行中にページを開き直した場合は操作が無く鳴らせないので、次の pointerdown/keydown で再挑戦する。決着（END）で0.9秒フェードアウト。効果音（blip）が埋もれないよう初期音量0.3。**自動化テストでは音が出せない**のはおさんぽと同じ。
...
- **ダンジョン＝問いに答えて倒す（2026-09-06）**: コマンド戦闘は「たたかう連打、ボスがためたら まもる」で終わり単調、かつエンジニアであることが戦闘に乗っていなかった（ヌルポやデッドロックガニを殴るだけ）。戦闘の入力を**問いへの解答**に置き換えた。
  - **正解＝こちらの攻撃／不正解＝敵の攻撃＋その場で解説**。発問から10秒以内の正解はクリティカル（`FAST_ANSWER_MS`・時間はサーバーの `askedAt` との差＝改ざん不能）。コマンドは **パス**（問いを見送り0.4倍で受ける・SP+2）／**ヒント**（SP3で選択肢を2つ消す・敵のターン無し・○×には効かない）／どうぐ・おふだ・にげる（従来どおり）。ボスの「ためる」は**次の問いが難問**になり、不正解は2.4倍・パスなら0.3倍・正解なら空振り。まちがいの一撃は素の0.75倍（`WRONG_MUL`）。
  - **出題元は2つ**: 良問バンク `QuizQuestion`（四択）と `src/lib/dungeon/riddles.ts`（○×60問のTSマスタ・浅い階の雑魚戦は50%で「○×高速ラウンド」`foe.rapid`、バンクが空のときの保険も兼ねる）。モンスターは `Monster.topics`（良問バンクの topic と同じ語彙・`topicMatches` のゆるい包含で照合）で得意領域の問題を出す。
  - **選定順**（`src/lib/dungeon/quiz-pool.ts` `pickQuestion`・純関数）: 復習ボックスの期限が来た問題（＝間違えた問題が「再戦」で戻る）→ 領域一致の未解答（良問スコア順）→ その他の未解答 → 解答済み。難易度は問題ごとの正答率（全員分・3件以上）で1〜3に分け、深度とためるで欲しい難易度に寄せる。同じ潜行では同じ問いを出さない（`DiveState.askedIds`）。
  - **良問バンクの記録に乗る**: ダンジョン内の解答も `recordAttempt`（`src/lib/quiz/attempt.ts`・腕試しと共通化）で `QuizAttempt` に記録し、復習ボックス・今日の一問・スキル検証（`promoteSkillsVerifiedByQuiz`、actions.ts から移動）に効く。ただし `QuizAttempt.source`（"quiz"/"dungeon"・マイグレーション `quiz_attempt_source`）で区別し、**EXP（exp.ts）と したく（prep.ts）は source="quiz" だけ**を数える＝ダンジョン内で稼いで次の潜行やEXPを強くする循環を切る。ダンジョンはEXP対象外のまま。
  - 判定は全部サーバー（`session-actions.ts`）。クライアントは選択肢の index とコマンド名だけ送る。`DiveState.foe.question` は正解を持たない（state はDBに残るため）。問いの選定はDBが要るので `ensureQuestion` がアクションの最後に行い、`session.ts` は `askQuestion`/`doAnswer`/`doBattle` の純関数だけ。旧形式の ACTIVE な潜行（問い無しの BATTLE）は `getActiveDive` が問いを補って続きから遊べる。
  - **問題の補充**: リアルタイム生成はしない（日次上限を食う）。`npm run gen:quiz -- --topic "SQL" -n 10` を領域（HTTP/SQL/Linux/Git/セキュリティ/Docker/テスト/設計/プロジェクト/Web/AWS）ごとに回して作り置きする。
...
- **ダンジョン＝ウィザードリィ風の一人称探索（2026-09-09）**: 残っていた「画面が固定構図」の単調さを、迷路を歩く一人称ビューで解消した。Artifact の試作「迷宮ウォーカー」（https://claude.ai/code/artifact/ad1da3f1-3354-417f-b586-299db45efe9c）でユーザー確認済みの作りをそのまま移植。
  ...
  - **歩く体験の調整（2026-09-10・ユーザー実機フィードバック）**: 「一歩ごとにイベントが挟まる／似たイベントが頻発」。原因は①イベントの無いマスでも毎歩フレーバーの一言を返し、UIが文字送りの間 十字キーを隠していた（実質 毎歩がイベント）②9×9 に5個前後のイベントが密集③浅い階の敵は候補5種の重み抽選で連続しやすい。対応: **EXPLORE のログは文字送りしない**（`apply` で即時表示・logs が空なら前の表示を残す）、一言は `STROLL_RATE`=22% で直前と別の文（12種）、迷路を **11×11** にしてイベント同士を `EVENT_SPACING`=3 以上離す、同じ敵の連続を避ける（`lastFoeId`）。バランスは変わらず（下の数値と同じ）。
  - ⚠ **「猫が」で文字送りが無限ループ（2026-09-10）**: PR #48 の「進む」長押しは 240ms ごとに move を送るが、休憩マス（猫）に入って十字キーが消えると pointerup がボタンに届かず止まらない。サーバーはイベント中の move を無視して同じ状態を返し、クライアントが毎回 `apply` で文字送りをやり直していた。対応: 無視した行動は `act` が `noop: true` を返し `send` は `apply` しない／探索フェーズを抜けたら長押しを必ず止める（`phaseRef`＋effect）。**教訓: 「同じ状態を返す」応答でも UI をリセットしてはいけない。**
...
- **ダンジョン＝出題のバリエーション（2026-09-11・ユーザー実機フィードバック「敵と出会った時の問題の種類が少なすぎる」）**: 原因は3つ。①○×マスタが60問で、モンスターの得意領域のうち テスト・Docker・AWS・プロジェクト・Web には専用問題がゼロだった（`onTopic` が空→全体からの抽選に落ち、敵の個性が消えていた）②四択はDBの良問バンク頼みで、領域が合わなければ「その他の未解答」（seedの問題は パフォーマンス/エクセル等で領域が合わない）が出て、誰と戦っても同じ問題になっていた ③潜行をまたいだ記憶が無く、翌日も同じ○×が出ていた。さらに `pickQuestion` が askedIds を素のIDで照合していて（保存は `bank:<id>`）、同じ潜行で同じバンク問題が繰り返し出るバグもあった。
  - **TSマスタ `src/lib/dungeon/riddles.ts` を ○×130問＋四択66問に**（`Riddle` は `kind: "truefalse" | "choice"` の判別共用体。`RIDDLES` は両方を合わせた配列）。全11領域を各 ○×10〜14・四択6 で網羅。`quiz-pool.test.ts` が「どのモンスターにも ○×20問以上・四択10問以上」「○×の偏り」「四択の正解位置の偏り（いつもB にしない）」を見張る。
  - **選定順を `pickBattleQuestion`（純関数）に集約**: 復習の再戦 → 領域一致の未解答バンク → **領域一致のTSマスタ（新顔のみ）** → その他のバンク → TSマスタ全体。「領域一致のTSマスタ」を「その他のバンク」より前に置いたのが変更点で、敵の得意領域が必ず出題に乗る。○×高速ラウンド（`foe.rapid`）は従来どおり ○× だけ。TSマスタの四択はヒント（選択肢2つ消し）も効く（`riddleAnswerIndex`）。
  - **直近の潜行で出した問いを後回し**: `DungeonRun.askedIds`（マイグレーション `dungeon_run_asked_ids`・決着時に state から写す）を `loadRecentAsked` が直近 `RECENT_RUNS`=6 回ぶん読み、`recentIds` として選定に渡す（除外ではなく後回し。新顔が尽きたら出る）。
  - 数値バランスは触っていない（`KIND_MUL` 等そのまま）。TSマスタの四択は難易度2（ふつう）扱い。
